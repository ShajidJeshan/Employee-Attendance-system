...
class LogInController extends Controller
{
    ...
    public function logIn(Request $req)
    {
        ...
        if (!$user) {
            return response()->json([
                'code'=>'5',
                'message' => 'Invalied employee id or password']);
        }
        else{
            // remove previous emp_id and password
            $req->request->remove('$req->emp_id');
            $req->request->remove('$req->password');
            ...
            $request->headers->set('Access-Control-Expose-Headers', $token);
